...
export default function Credits() {
  ...
  // ── FIX PRINCIPAL : onAuthStateChange écoute SIGNED_IN + INITIAL_SESSION
  //    Utilise localStorage (résiste aux redirections OAuth sur mobile)
  useEffect(() => {
    // Nettoyer le ?pending=true de l'URL sans recharger la page
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        // Réagir aussi bien à SIGNED_IN qu'à INITIAL_SESSION
        // (iOS Safari / certains navigateurs restaurent la session via INITIAL_SESSION)
        if (
          (event === 'SIGNED_IN' || event === 'INITIAL_SESSION') &&
          session?.user?.id
        ) {
          const pendingPack = getPendingPack();
          if (!pendingPack) return; // Pas de pack en attente — ne rien faire

          // Nettoyer immédiatement pour éviter tout double-déclenchement
          clearPendingPack();
  ...
  // ── FIX : utiliser uniquement getSession() — plus de clé localStorage hardcodée
  const handleBuy = async () => {
    ...
    try {
      ...
      if (!session?.user?.id) {
        // Pas connectée → ouvrir la modale OAuth
        setLoading(false);
        setShowAuthModal(true);
        return;
      }

      // Connectée → lancer FedaPay directement
