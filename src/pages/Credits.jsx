...
async function callFedaPay(pack, userId, email = '') {
  // Si pas connectée, utiliser le fingerprint comme sessionId
  const fp = localStorage.getItem('afrotresse_fp');
  const sessionId = userId || (fp ? `fp_${fp}` : null);

  if (!sessionId) {
    return { error: 'Identifiant introuvable. Veuillez reessayer.' };
  }
...
export default function Credits() {
  ...
  useEffect(() => {
    ...
    if (params.get('payment') === 'success') {
      ...
      // Poller jusqu'à ce que le webhook ait crédité (max 30s, toutes les 3s)
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (synced != null && synced > creditAvantPaiement) return; // Webhook traité ✓
        } catch {}
        if (attempts < 10) setTimeout(poll, 3000); // Réessayer dans 3s
      };
      setTimeout(poll, 2000); // Premier essai après 2s
    }
  }, []);
