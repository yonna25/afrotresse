...
function FedaPayModal({ url, onClose, onSuccess }) {
  ...
  // Évite que le callback_url charge AfroTresse dans l'iframe (double fenêtre)
  useEffect(() => {
    let attempts = 0;
    let timer;
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (synced != null && synced > before) {
          onSuccess(); // Paiement confirmé → fermer iframe + afficher succès
          return;
        }
      } catch {}
    ...
    // Démarrer le polling 3s après ouverture de l'iframe
...
async function callFedaPay(pack, userId, email = '') {
  ...
  if (userId) {
    ...
    // Charger le fingerprint de façon async — jamais de guest_user
