...
export default function AdminPartners() {
  ...
  const [approving, setApproving]       = useState(null); // id of request being approved
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      else { setIsAdmin(true); fetchPartners(); fetchRequests(); }
    };
    checkAuth();
  }, []);
  ...
  const handleApprove = async (req, isPremium = false) => {
    if (approving === req.id) return; // anti double-clic
    setApproving(req.id);
    try {
      ...
      if (insertError) { alert("Erreur insertion : " + insertError.message); return; }
      ...
      const { error: updateError } = await supabase
        ...
        .eq("id", req.id);
      if (updateError) { alert("Erreur statut : " + updateError.message); return; }
                  ...
                  {req.status === "pending" && (
                    <div className="flex gap-2 flex-wrap">
                      <button
                        ...
                        className={`flex-1 py-2.5 rounded-xl text-[10px] font-black border transition-all ${approving === req.id ? "bg-white/5 text-white/20 border-white/10 cursor-not-allowed" : "bg-[#C9963A]/15 text-[#C9963A] border-[#C9963A]/25 hover:bg-[#C9963A]/25"}`}
                      >
                        {approving === req.id ? "⏳…" : "✅ Freemium"}
                      </button>
                      <button
                        onClick={() => handleApprove(req, true)}
                        disabled={approving === req.id}
                        className={`flex-1 py-2.5 rounded-xl text-[10px] font-black border transition-all ${approving === req.id ? "bg-white/5 text-white/20 border-white/10 cursor-not-allowed" : "bg-yellow-500/15 text-yellow-400 border-yellow-500/25 hover:bg-yellow-500/25"}`}
                      >
                        {approving === req.id ? "⏳…" : "👑 Premium"}
                      </button>
                      <button
                        onClick={() => handleReject(req.id)}
                        className="w-full py-2.5 rounded-xl text-[10px] font-black bg-red-500/15 text-red-400 border border-red-500/25 hover:bg-red-500/25 transition-all"
                      >
                        ❌ Rejeter
                      </button>
                    </div>
