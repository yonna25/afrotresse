...
import { getCredits, consumeTransform, hasCredits, canTransform, addSeenStyleId, getSeenStyleIds, PRICING, consumeCredits } from "../services/credits.js";
...
// ═══════════════════════════════════════════════════════════════════════════
// SPARKLE COMPONENT — Étincelles magiques
// ═══════════════════════════════════════════════════════════════════════════
function Sparkle({ id }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 0, x: x, y: y, scale: 0 }}
    ...
    >
      <div className="w-1 h-1 bg-[#E8B96A] rounded-full shadow-lg" style={{
        boxShadow: "0 0 8px #C9963A, 0 0 4px #E8B96A"
      }} />
    </motion.div>
  );
}
...
export default function Results() {
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // HANDLE GET NEW STYLES — Générer 3 nouveaux styles
  // ═════════════════════════════════════════════════════════════════════════
  const handleGetNewStyles = () => {
    ...
    // 🎆 Déclencher l'animation sparkle
    ...
    // Récupérer les styles vus
    const seenIds = getSeenStyleIds();
    
    // Filtrer les styles par forme de visage et exclure les vus
    const available = BRAIDS_DB.filter(s => 
      s.faceShapes.includes(faceShape) && !seenIds.includes(s.id)
    );

    // Si moins de 3 styles disponibles, réinitialiser les "vus"
    if (available.length < PAGE_SIZE) {
      const filtered = BRAIDS_DB.filter(s => s.faceShapes.includes(faceShape));
      const shuffled = [...filtered].sort(() => Math.random() - 0.5).slice(0, PAGE_SIZE);
      shuffled.forEach(s => addSeenStyleId(s.id));
      setStyles(shuffled);
    } else {
      // Mélanger et prendre les 3 premiers
      const shuffled = [...available].sort(() => Math.random() - 0.5).slice(0, PAGE_SIZE);
      shuffled.forEach(s => addSeenStyleId(s.id));
      setStyles(shuffled);
    }
  ...
  return (
    <div className="min-h-[100dvh] bg-[#2C1A0E] text-[#FAF4EC] p-4 sm:p-6 pb-40 relative">
      ...
      {/* FLOATING GENERATE BUTTON — LE BOUTON MAGIQUE */}
      <motion.div
        initial={{ y: 100, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        onClick={handleGetNewStyles}
        disabled={isGenerating}
        className="fixed bottom-40 right-5 z-39 relative"
      >
        <motion.button
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
          className="relative w-16 h-16 rounded-full shadow-2xl border-2 border-[#2C1A0E]/30 flex items-center justify-center font-black text-2xl active:scale-95 transition-all disabled:opacity-60 cursor-pointer overflow-hidden group"
          style={{
            background: 'linear-gradient(135deg, #C9963A, #E8B96A)',
            boxShadow: '0 0 30px rgba(201,150,58,0.5), 0 0 60px rgba(201,150,58,0.3)',
          }}
          disabled={isGenerating}
        >
          {/* Shimmer effect */}
          ...
          <div className="relative flex flex-col items-center justify-center">
            {isGenerating ? (
              ...
              >
                ...
                <path d="M12 2v6m0 4v6m-8-6h6m4 0h6" />
              </motion.svg>
            ) : (
              <>
                <span className="text-sm leading-none">✨</span>
                <span className="text-[10px] font-black leading-tight mt-0.5">3</span>
              </>
            )}
          </div>
          ...
          <motion.div
            ...
            transition={{ duration: 2, repeat: Infinity }}
            className="absolute inset-0 rounded-full border-2 border-[#E8B96A] opacity-30"
            style={{ boxShadow: '0 0 20px rgba(232,185,106,0.5)' }}
          />
        </motion.button>
