...
export default function Results() {
  ...
  useEffect(() => {
    ...
    // Charge local d'abord, corrige avec serveur ensuite
    const localC = getCredits();
    setCredits(localC);
    syncCreditsFromServer()
      .then(c => setCredits(c))
      .catch(() => {/* garde le local */});
  }, []);
  ...
  // ── Générer plus — utilise les crédits locaux, pas de redirection silencieuse
  const handleGenerateMore = async () => {
    ...
    try {
      // 1. Lire le solde local (source fiable pour les crédits session/free)
      const localCredits = getCredits();

      // 2. Essayer de syncer avec le serveur, mais sans bloquer si ça échoue
      let effectiveCredits = localCredits;
      try {
        const serverCredits = await syncCreditsFromServer();
        // Prendre le MAX entre local et serveur — l'un des deux peut être en retard
        effectiveCredits = Math.max(localCredits, serverCredits ?? 0);
        setCredits(effectiveCredits);
      } catch {
        // Sync échoué → on continue avec les crédits locaux
        effectiveCredits = localCredits;
      }

      // 3. Vérifier le solde effectif
      ...
      // 5. Mettre à jour l'affichage du solde
      setCredits(getCredits());

      // 6. Débloquer la page suivante
      ...
      <div className="min-h-[100dvh] bg-[#1A0A00] text-[#FAF4EC] flex flex-col relative overflow-hidden">
        ...
        <div className="flex flex-col flex-1 px-5 pt-2 pb-32">
          ...
          <div className="flex flex-col gap-3 mb-8">
            {[
              { icon: "📸", label: "Prends un selfie",    sub: "Ou uploade une photo" },
              { icon: "🔍", label: "Analyse instantanée", sub: "Tes proportions en secondes" },
              { icon: "✨", label: "Styles sur-mesure",   sub: "3 recommandations pour toi" },
            ].map((step, i) => (
              <div key={i} className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl px-4 py-3">
                <span className="text-2xl">{step.icon}</span>
                <div>
                  <p className="text-sm font-bold text-white">{step.label}</p>
                  <p className="text-[10px] text-white/40">{step.sub}</p>
                </div>
                <div className="ml-auto w-6 h-6 rounded-full bg-[#C9963A]/20 border border-[#C9963A]/40 flex items-center justify-center text-[#C9963A] text-[10px] font-black">
                  {i + 1}
                </div>
              </div>
            ))}
  ...
  return (
    <div className="min-h-[100dvh] bg-[#1A0A00] text-[#FAF4EC] p-4 sm:p-6 pb-40 relative">
      ...
      {/* Message d'erreur inline — plus de redirection silencieuse */}
      {errorMsg && (
        <motion.div
          ref={errorRef}
          initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          className="mb-4 px-5 py-4 rounded-2xl border text-center"
          style={{ background: "rgba(201,150,58,0.08)", borderColor: "rgba(201,150,58,0.3)" }}>
          <p className="text-[#C9963A] text-[12px] font-bold mb-3">{errorMsg}</p>
          <button
            onClick={() => navigate("/credits")}
            className="px-6 py-2 rounded-xl font-black text-[11px] text-[#2C1A0E] uppercase tracking-widest"
            style={{ background: "linear-gradient(135deg, #C9963A, #E8B96A)" }}>
            ...
            <motion.div key={style.id || index}
              ...
              <div className="p-6 text-center">
                <div className="flex justify-between items-center mb-4 px-2">
                  ...
                  <button onClick={() => handleToggleFav(style)} className="text-xl">
                    {isFavorited ? "❤️" : "🤍"}
                  </button>
                </div>
