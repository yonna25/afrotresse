...
export default function Results() {
  ...
  const [isTesting, setIsTesting]         = useState(false);        // verrou + feedback visuel pendant l'appel API
  const [tryOnStyleId, setTryOnStyleId]   = useState(null);         // identifie quelle card est en cours de génération
  const tryOnAbortRef                     = useRef(null);            // AbortController pour annuler si démontage
  ...
  const handleTestTryOn = async (style) => {
    // Guard : un seul appel à la fois
    if (isTesting) return;

    // Construction du prompt : nom du style + forme de visage + selfie en contexte textuel
    const styleLabel = style.name || "afro braids";
    const prompt = `Portrait of a woman wearing ${styleLabel}, face shape ${faceShape}, professional studio lighting, photorealistic`;

    setIsTesting(true);
    setTryOnStyleId(style.id);
    setErrorMsg("");

    // AbortController pour éviter les setState post-démontage
    ...
    try {
      ...
      if (!data.image) throw new Error("L'IA n'a pas retourné d'image.");

      // Affichage du résultat dans le modal de zoom existant
      setZoomImage(data.image);

    } catch (err) {
      if (err.name === "AbortError") return; // Démontage du composant — silencieux
      setErrorMsg(`✨ Essai virtuel : ${err.message}`);
  ...
  return (
    <div className="min-h-[100dvh] bg-[#1A0A00] text-[#FAF4EC] p-4 sm:p-6 pb-40 relative">
      ...
      {errorMsg && (
        <motion.div ref={errorRef} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mb-4 px-5 py-3 rounded-2xl bg-red-900/30 border border-red-500/30 text-red-300 text-[11px] font-bold text-center">
          {errorMsg}
        </motion.div>
      )}
            ...
            <motion.div key={style.id || index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-[#3D2616] rounded-[2.5rem] overflow-hidden border border-[#C9963A]/20 shadow-2xl">
              ...
              <div className="p-6">
                ...
                <button
                  ...
                  disabled={isTesting}
                  className="w-full py-4 rounded-2xl bg-white/5 border border-white/10 text-white/40 text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isTesting && tryOnStyleId === style.id ? (
                    <span className="flex items-center justify-center gap-2">
                      <svg className="animate-spin h-3 w-3 text-white/40" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      {showCatalogueEnd && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="mt-10 p-8 rounded-[3rem] bg-gradient-to-br from-[#2C1A0E] to-[#1A0A00] border border-[#C9963A]/30 text-center shadow-2xl">
          <div className="text-4xl mb-4">✨</div>
          <h3 className="text-xl font-black mb-2 text-[#C9963A]">Fin du catalogue !</h3>
          <p className="text-[11px] text-white/50 mb-8 leading-relaxed px-4 text-balance">Tu as vu toutes les recommandations actuelles pour ton profil. Tu peux mélanger le catalogue pour découvrir d'autres pépites.</p>
      ...
      <AnimatePresence>
        {zoomImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setZoomImage(null)} className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative max-w-full max-h-full">
