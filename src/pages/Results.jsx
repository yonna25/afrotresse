...
export default function Results() {
  ...
  const [saveCount, setSaveCount] = useState(0); // Compteur pour 1 credit = 3 saves
  ...
  // Logique : Afficher strictement 3 modèles par analyse (Anti-répétition)
  ...
  const handleSave = (imageUrl) => {
    ...
    // Gestion de la consommation : 1 credit tous les 3 saves
  ...
  return (
    <div className="min-h-screen bg-[#2C1A0E] text-[#FAF4EC] p-5 pb-32 overflow-x-hidden relative">
      ...
      {/* HEADER OPTIMISÉ : Selfie + Nom sur deux lignes */}
      <div className="mb-10 flex flex-row gap-5 items-center bg-white/5 p-5 rounded-[2.5rem] border border-white/10 shadow-xl">
        ...
        <div className="flex flex-col flex-1">
          {/* Titre demandé : Ton résultat <br/> Nom */}
          <h1 className="font-display font-bold text-xl text-[#C9963A] leading-tight">
            Ton r\u00e9sultat
            <br/>
            <span className="text-3xl text-[#FAF4EC] font-black">{userName} \u2728</span>
          </h1>
      ...
      {/* LISTE DES 3 STYLES (Grid 3 photos intouchée) */}
        ...
        {currentResults.map((style) => (
          <div key={style.id} className="bg-[#3D2616] rounded-[2.5rem] overflow-hidden border border-[#C9963A]/20 shadow-2xl relative">
            ...
            <div className="px-6 py-3 flex gap-5 text-[10px] font-black uppercase tracking-widest text-[#C9963A]/80 border-b border-white/5">
              <span>\ud83d\udc41 2.4K Vues</span> <span>\u2764 892 Likes</span>
            </div>

            <div className="p-6">
              <div className="flex justify-between items-start mb-3">
                <h3 className="font-display font-bold text-xl">{style.name}</h3>
                <span className="text-[10px] bg-[#C9963A] text-[#2C1A0E] px-3 py-1 rounded-full font-black uppercase">{style.duration}</span>
              </div>
      ...
      {/* BOUTON FLOTTANT SOLDE CRÉDIT (Nouveau format Sticker Bas Droite) */}
      <motion.div 
        initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
        onClick={() => navigate("/credits")}
        className="fixed bottom-24 right-4 z-40 bg-[#C9963A] text-[#2C1A0E] w-16 h-16 rounded-2xl flex flex-col items-center justify-center shadow-2xl border-2 border-[#2C1A0E]/20 active:scale-95 transition-all"
      >
        <div className="text-[8px] font-black uppercase opacity-60">Solde</div>
