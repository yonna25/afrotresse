...
export default function Results() {
  ...
  useEffect(() => {
    // 1. Récupération robuste des résultats
  ...
  [span_2](start_span)// Calcul des styles à afficher (Cumulatif pour éviter l'écran vide)[span_2](end_span)
  const stylesToDisplay = styles.slice(0, unlockedPages * STYLES_PER_PAGE);
  const totalPossiblePages = Math.ceil(styles.length / STYLES_PER_PAGE);
  ...
  // Sécurité : si pas de styles du tout, on affiche un message d'attente plutôt que rien
  if (styles.length === 0) {
    return (
      <div className="min-h-screen bg-[#1A0A00] flex items-center justify-center p-10 text-center">
        <p className="text-[#C9963A]">Analyse en cours ou session expirée...<br/>Redirection vers l'accueil.</p>
        <button onClick={() => navigate("/camera")} className="mt-4 text-white underline">Recommencer</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[100dvh] bg-[#1A0A00] text-[#FAF4EC] p-4 pb-40 relative overflow-x-hidden">
      ...
      {/* HEADER (Design exact de ton screenshot) */}
      <div className="mb-10 flex flex-row gap-5 items-center bg-white/5 p-5 rounded-[2.5rem] border border-white/10 shadow-2xl">
        <div className="relative shrink-0">
          ...
          <div className="absolute -bottom-2 -right-2 bg-[#C9963A] text-[#2C1A0E] text-[8px] font-black px-2 py-1 rounded uppercase">Moi</div>
        </div>
      ...
      [span_3](start_span){/* LISTE DES STYLES (Grilles de 3 images avec Zoom)[span_3](end_span) */}
      <div className="flex flex-col gap-8">
        {stylesToDisplay.map((style, index) => {
          const styleKey = style.id?.replace(/-/g, "");
            ...
            <motion.div key={style.id || index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#3D2616] rounded-[2.5rem] overflow-hidden border border-[#C9963A]/20 shadow-2xl">
              <div className="grid grid-cols-3 gap-0.5 h-72 bg-black/40">
                <div className="col-span-2 h-full overflow-hidden">
                  <img src={faceImg} className="w-full h-full object-cover cursor-zoom-in" onClick={() => setZoomImage(faceImg)} alt="Face" />
                </div>
                <div className="col-span-1 grid grid-rows-2 gap-0.5">
                  <img src={backImg} className="w-full h-full object-cover cursor-zoom-in" onClick={() => setZoomImage(backImg)} alt="Back" />
                  <img src={topImg} className="w-full h-full object-cover cursor-zoom-in" onClick={() => setZoomImage(topImg)} alt="Top" />
                </div>
              </div>
              <div className="p-6">
                ...
                <p className="text-[11px] opacity-60 leading-relaxed">{style.description || "Un style sur mesure pour ton visage."}</p>
              </div>
            </motion.div>
          );
        })}
      ...
      [span_4](start_span){/* BOUTON GÉNÉRER SOUS LA LISTE[span_4](end_span) */}
      {unlockedPages < totalPossiblePages && (
      ...
      [span_5](start_span){/* BOUTONS FLOTTANTS (Design exact screenshot)[span_5](end_span) */}
      <div className="fixed bottom-24 right-4 z-50 flex flex-col items-center gap-3">
        <div onClick={() => navigate("/credits")} className="w-12 h-12 bg-[#C9963A] text-[#2C1A0E] rounded-xl flex flex-col items-center justify-center shadow-2xl border border-[#2C1A0E]/20 cursor-pointer active:scale-90">
      ...
      [span_6](start_span){/* MODAL ZOOM (Rétabli)[span_6](end_span) */}
      <AnimatePresence>
        {zoomImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setZoomImage(null)} className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <motion.img initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} src={zoomImage} className="max-w-full max-h-[80vh] rounded-3xl shadow-2xl border border-white/10 object-contain" />
