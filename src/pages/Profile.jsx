  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#1A0A00] text-white flex flex-col items-center pb-32 relative">
        <AnimatePresence>
          {toastMsg && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed top-5 left-1/2 -translate-x-1/2 z-[300] bg-[#C9963A] text-[#1A0A00] px-5 py-3 rounded-2xl font-black text-sm shadow-2xl">
              {toastMsg}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="w-full relative">
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2">
            <div className="w-24 h-24 rounded-full border-4 border-[#C9963A] overflow-hidden bg-[#2a1a14] shadow-2xl">
              {selfieUrl ? <img src={selfieUrl} className="w-full h-full object-cover" alt="Profil" /> : <div className="w-full h-full flex items-center justify-center text-4xl">👑</div>}
            </div>
          </div>
        </div>
              ...
              <AnimatePresence>
                {showLoginForm && (
                  <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="px-4 pb-4 overflow-hidden">
                    <button onClick={() => navigate("/magic-link")} className="w-full py-3 rounded-xl font-black text-sm text-[#1A0A00] bg-[#C9963A]">Continuer par Email ✨</button>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
        ...
        <div className="w-full max-w-sm px-5 mt-6">
          <div className="rounded-3xl border border-[#C9963A]/30 bg-[#1A0A00]">
            <button onClick={() => setShowReferralInfo(!showReferralInfo)} className="w-full p-5 flex items-center justify-between">
              <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-2xl bg-[#C9963A]/20 flex items-center justify-center">🎁</div><div><p className="font-black text-sm">Parrainage</p></div></div>
