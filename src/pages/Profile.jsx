...
export default function Profile() {
  ...
  const [activeTab, setActiveTab] = useState("styles"); // "styles" ou "essais"
  ...
  return (
    <div className="min-h-screen bg-[#1a0f0a] text-white flex flex-col items-center px-4 py-6 pb-32">
      ...
      {/* 1. HEADER : PHOTO RÉCUPÉRÉE + BADGE COURONNE */}
      <div className="flex flex-col items-center gap-2 mt-4">
        <div className="relative">
          ...
          {/* Badge Couronne déporté sur le côté pour le style Premium */}
      ...
      <div className="flex bg-[#2a1a14] rounded-2xl mt-10 p-1 w-full max-w-sm border border-white/5">
        <button 
          ...
          className={`flex-1 py-3 rounded-xl text-xs font-bold uppercase transition-all ${activeTab === 'styles' ? 'bg-[#C9963A] text-[#1a0f0a]' : 'text-gray-500'}`}
        >
          Styles
        </button>
        <button 
          onClick={() => setActiveTab("essais")}
          className={`flex-1 py-3 rounded-xl text-xs font-bold uppercase transition-all ${activeTab === 'essais' ? 'bg-[#C9963A] text-[#1a0f0a]' : 'text-gray-500'}`}
        >
          Essais
        </button>
      </div>

      {/* 4. CONTENU DYNAMIQUE SELON L'ONGLET */}
      <div className="w-full mt-12 flex flex-col items-center">
        {activeTab === "styles" ? (
          ...
          /* ÉTAT VIDE POUR "ESSAIS" (NE PAS LAISSER VIDE) */
          <div className="text-center px-10">
            <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-4 mx-auto border border-white/10 text-xl">📸</div>
      ...
      {/* 5. INFORMATIONS LÉGALES DISCRÈTES (PIED DE PAGE) */}
