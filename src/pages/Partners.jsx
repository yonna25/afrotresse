...
function Modal({ partner, onClose }) {
  ...
  const isPremium = partner.is_premium || partner.is_featured || false;

  const wa = () => {
    if (!isPremium) return; // bloqué si freemium
    window.open(
          ...
          <div style={{ padding:"0 24px 16px" }}>
            {isPremium ? (
              ...
              <div style={{
                ...
                }}>🔒 Partenaire Freemium</p>
                <p style={{
                  fontFamily:"'Jost', sans-serif",
                  fontSize:11, color:T.inkLight,
                  lineHeight:1.5,
                }}>
                  Ce partenaire n'a pas encore activé<br/>son accès Premium AfroTresse
                </p>
              </div>
            )}
          </div>
...
// ─── FREEMIUM CARD ────────────────────────────────────────────────────────────
function FreemiumCard({ partner }) {
  // Affiche les 4 premiers chiffres visibles, masque le reste
  const phone = (partner.whatsapp || partner.phone || "").replace(/\D/g, "");
  const countryCode = phone.startsWith("229") ? "+229 " : phone.length > 8 ? `+${phone.slice(0,3)} ` : "";
  const local = phone.startsWith("229") ? phone.slice(3) : phone;
  const visible = local.slice(0, 2);
  const masked = local.length > 2 ? " ••••••" : " ••••••";
  const displayPhone = phone ? `${countryCode}${visible}${masked}` : "+229 64 ••••••";

  const specialties = Array.isArray(partner.specialties)
    ? partner.specialties
    : (partner.description || "").replace("Spécialités : ", "").split(", ").filter(Boolean);
    ...
    <div style={{
      ...
      <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:12 }}>
        ...
        <div style={{
          ...
          background:`linear-gradient(145deg, ${T.bgDeep}, ${T.cream})`,
          border:`1.5px solid rgba(200,135,58,0.12)`,
          display:"flex", alignItems:"center", justifyContent:"center",
          fontSize:24,
        }}>{partner.emoji || "👑"}</div>
        ...
        <div style={{
          ...
          border:`1px dashed ${T.amberLine}`,
          fontFamily:"'Jost', sans-serif",
          fontSize:8, fontWeight:700,
          color:T.inkFade, letterSpacing:"0.1em",
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:12 }}>
          ...
          {specialties.length > 4 && (
            <span style={{
              ...
              background:T.bgDeep,
              fontFamily:"'Jost', sans-serif",
              fontSize:8, color:T.inkFade,
            }}>+{specialties.length - 4}</span>
          )}
        </div>
      )}

      {/* WhatsApp partiellement visible */}
      <div style={{
...
function PartnerForm() {
  ...
  const validateStep2 = () => {
    const e = {};
    if (form.specialties.length === 0) e.specialties = "Sélectionne au moins une spécialité";
    setErrors(e);
    return Object.keys(e).length === 0;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ submit: "Erreur lors de l\'envoi. Réessaie." });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const inputStyle = (err) => ({
    ...
    borderRadius:12,
    border:`1.5px solid ${err ? "#E53E3E" : T.amberLine}`,
    background:T.bgDeep,
    fontFamily:"'Jost', sans-serif",
  ...
  const labelStyle = {
    ...
    color:T.inkLight, marginBottom:6, display:"block",
  };
    ...
    <div style={{ width:"100%" }}>
      ...
      <button
        ...
        style={{
          ...
          borderRadius:16,
          border:`1.5px solid ${open ? T.amber : T.amberLine}`,
          background: open ? T.amberDim : T.bgDeep,
      ...
      {open && (
        <div style={{
          ...
          <div style={{ marginBottom:20 }}>
            <div style={{
              ...
              {[1,2,3,4].map(n => (
                <div key={n} style={{
                  width:36, height:36, borderRadius:"50%",
                  display:"flex", alignItems:"center", justifyContent:"center",
                  background: step >= n ? T.amber : T.bgDeep,
                  border:`1.5px solid ${step >= n ? T.amber : T.amberLine}`,
                  fontFamily:"'Jost', sans-serif",
                  fontSize:12, fontWeight:700,
                  color: step >= n ? T.white : T.inkLight,
            ...
            }}>Étape {step} sur {TOTAL_STEPS}</div>
          </div>
                  ...
                  {CATEGORIES_FORM.map(cat => (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius:12,
                        border:`1.5px solid ${form.category === cat.id ? T.amber : T.amberLine}`,
                        background: form.category === cat.id ? T.amberDim : T.bgDeep,
                        cursor:"pointer",
                        fontFamily:"'Jost', sans-serif",
                        fontSize:11, fontWeight:600,
                        color: form.category === cat.id ? T.amber : T.inkLight,
                        transition:"all 0.2s",
                        textAlign:"center",
                      }}
                    >
          ...
          {step === 2 && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <p style={{
                fontFamily:"'Jost', sans-serif",
                fontSize:12, color:T.inkLight, marginBottom:4,
              }}>Sélectionne au moins une spécialité proposée</p>

              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {["Box Braids","Cornrows","Knotless Braids","Fulani Braids",
                  "Goddess Braids","Twists","Vanilles","Locks","Faux Locs",
                  "Crochet Braids","Senegalese Twists","Micro Braids"].map(s => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        padding:"7px 14px",
                        borderRadius:99,
                        border:`1.5px solid ${active ? T.amber : T.amberLine}`,
                        background: active ? T.amberDim : T.bgDeep,
                        fontFamily:"'Jost', sans-serif",
                        fontSize:11, fontWeight:600,
                        color: active ? T.amber : T.inkLight,
              ...
              {errors.specialties && (
                <p style={{ fontFamily:"'Jost', sans-serif", fontSize:10, color:"#E53E3E" }}>
                  {errors.specialties}
                </p>
              )}
              ...
              <div style={{ display:"flex", gap:10, marginTop:4 }}>
                <button onClick={() => setStep(s => s - 1)} style={{
                  ...
                  background:T.bgDeep,
                  fontFamily:"'Jost', sans-serif",
                  fontSize:12, color:T.inkMid, cursor:"pointer",
                }}>← Retour</button>
          ...
          {step === 3 && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div>
                ...
                <input
                  type="tel"
                  placeholder="Ex: +229 64 00 00 00"
                  value={form.whatsapp}
              ...
              <div>
                ...
                <input
                  type="email"
                  placeholder="Ex: monsalon@email.com"
                  value={form.email}
          ...
          {step === 4 && !submitted && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              {[
                ...
                ["Spécialités", form.specialties.join(", ")],
              ...
              {errors.submit && (
                <p style={{ fontFamily:"'Jost', sans-serif", fontSize:11, color:"#E53E3E", textAlign:"center" }}>
                  {errors.submit}
                </p>
              )}
          ...
          {submitted && (
            <div style={{ textAlign:"center", padding:"28px 0" }}>
              ...
              <p style={{
                ...
                Nous examinerons ta demande et te contacterons<br/>
                par email sous 48h. Merci de rejoindre AfroTresse ✦
              </p>
            </div>
          )}
...
export default function Partners() {
  ...
  const premium   = filtered.filter(p => !p.is_featured && p.is_premium);
  const freemium  = filtered.filter(p => !p.is_featured && !p.is_premium);
        ...
        <div style={{ padding:"12px 20px 0", display:"flex", flexDirection:"column", gap:12 }}>
          {loading ? (
            ...
            <>
              ...
              {(featured.length > 0 || premium.length > 0) && freemium.length > 0 && (
              ...
              {freemium.map(p => (
                <FreemiumCard key={p.id} partner={p}/>
              ))}
            </>
          )}
        </div>
