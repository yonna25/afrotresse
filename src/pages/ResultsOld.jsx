...
export default function Results() {
      ...
      <div className="min-h-[100dvh] bg-[#1A0A00] text-[#FAF4EC] flex flex-col relative overflow-hidden">
        <div className="relative h-52 overflow-hidden bg-[#1A0A00] flex items-center justify-center">
          <div className="absolute inset-0 flex flex-col items-center justify-center"
            ...
            <motion.p className="text-white font-black text-2xl text-center px-4 leading-tight">Ton visage,<br /><span className="text-[#C9963A]">tes styles ✨</span></motion.p>
          </div>
        ...
        <div className="flex flex-col flex-1 px-5 pt-2 pb-32">
          ...
          <p className="text-[12px] text-white/50 leading-relaxed mb-6 italic">"Un selfie suffit pour trouver la coiffure qui te correspond."</p>
          <div className="flex flex-col gap-3 mb-8">
            {[{ icon: "📸", label: "Prends un selfie", sub: "Ou uploade une photo" }, { icon: "🔍", label: "Analyse instantanée", sub: "Tes proportions en secondes" }, { icon: "✨", label: "Styles sur-mesure", sub: "3 recommandations pour toi" }].map((step, i) => (
              <div key={i} className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl px-4 py-3">
                ...
                <div className="ml-auto w-6 h-6 rounded-full bg-[#C9963A]/20 border border-[#C9963A]/40 flex items-center justify-center text-[#C9963A] text-[10px] font-black">{i+1}</div>
              </div>
            ))}
          </div>
          <button onClick={() => navigate("/camera")} className="w-full py-5 rounded-2xl font-black text-lg text-[#2C1A0E]" style={{ background: "linear-gradient(135deg, #C9963A, #E8B96A)" }}>📸 Prendre mon selfie</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[100dvh] bg-[#1A0A00] text-[#FAF4EC] p-4 sm:p-6 pb-40 relative">
      ...
      {errorMsg && (
        <motion.div ref={errorRef} className="mb-4 px-5 py-3 rounded-2xl bg-red-900/30 border border-red-500/30 text-red-300 text-[11px] font-bold text-center">{errorMsg}</motion.div>
      )}
      ...
      <motion.div className="mb-10 flex flex-row gap-5 items-center bg-white/5 p-5 rounded-[2.5rem] border border-white/10" style={{ boxShadow: "0 0 40px rgba(201,150,58,0.2)" }}>
        <div className="relative shrink-0">
          <ProtectedImg src={selfieUrl} className="w-20 h-20 rounded-2xl border-2 border-[#C9963A] object-cover" />
